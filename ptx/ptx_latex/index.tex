...
Use a computer algebra system, such as \href{http://http://www.wolframalpha.com/}{Wolfram Alpha}, to verify your work. I suggest Wolfram Alpha, because it is now built into Mathematica 8.0+. If you can learn to use Wolfram Alpha, you will be able to use Mathematica.%
...
\end{aside}
\begin{task}[]\label{task-3}
Find the differential of \(y\) with respect to \(x\).%
\end{task}
\begin{task}[]\label{task-4}
Draw a graph of \(f(x)\). Place a dot at the point \((3,9)\) and label it on your graph. Sketch a tangent line to the graph at the point \((3,9)\) on the same axes. Place another dot on the tangent line up and to the right of (3,9). Label the point \((x,y)\), as it will represent any point on the tangent line.%
\end{task}
\begin{task}[]\label{task-5}
Using the two points \((3,9)\) and \((x,y)\), compute the slope of the line connecting these two points. Your answer should involve \(x\) and \(y\).%
\par
What is the rise (i.e, the change in \(y\) called \(dy\))? What is the run (i.e, the change in \(x\) called \(dx\))?%
\end{task}
\begin{task}[]\label{task-6}
We know the slope of the tangent line is the derivative \(f'(3)=6\). We also know the slope from the previous part. These two must be equal. Use this fact to give an equation of the tangent line to \(f(x)\) at \(x=3\). (Hint: do NOT simplify to slope-intercept form)%
\end{task}
\begin{task}[]\label{task-7}
How is the equation for the tangent line related to the differentials \(dy\) and \(dx\)?%
\end{task}
\end{exploration}
...
\end{aside}
\begin{task}[]\label{task-13}
\(\begin{cases}x+y\amp =3\\2x-y\amp =4
\end{cases}\)%
\end{task}
\begin{task}[]\label{task-14}
\(\begin{cases}-x + 4y\amp =8\\3x - 12y\amp =2
\end{cases}\)%
\end{task}
\bigbreak
\leavevmode%
...
\end{task}
\begin{task}[]\label{task-16}
If you let \(y=t\), what are \(x\) and \(z\) (in terms of \(t\)).%
\end{task}
\begin{task}[]\label{task-17}
...
\begin{task}[]\label{task-101}
Show that \(d=2a\) by considering the distances from \(F_1\) and \(F_2\) to the point \(Q=(a,0)\).%
\end{task}
\begin{task}[]\label{task-102}
Let \(Q=(0,b)\) be a point on the ellipse.  Show that \(b^2+c^2=a^2\) by considering the distance between \(Q\) and each focus.%
\end{task}
\begin{task}[]\label{task-103}
Let \(Q=(x,y)\). By considering the distances between \(Q\) and the foci, show that an equation of the ellipse is%
\begin{equation*}
\frac{x^2}{a^2}+\frac{y^2}{b^2}=1.
\end{equation*}
%
\end{task}
\begin{task}[]\label{task-104}
Suppose the foci are along the \(y\)-axis (at \((0,\pm c)\)) and the fixed distance \(d\) is now \(d=2b\), with vertexes \((0,\pm b)\). Let \((a,0)\) be a point on the \(x\) axis that intersect the ellipse.  Show that we still have%
\begin{equation*}
\frac{x^2}{a^2}+\frac{y^2}{b^2}=1,
\end{equation*}
...
\begin{task}[]\label{task-105}
\(\ds 16x^2+25y^2=400\) divide by 400.%
%
\end{task}
\begin{task}[]\label{task-106}
...
\begin{task}[]\label{task-107}
The foci are at \((2\pm 3,1)\) and vertices at \((2\pm 5, 1)\).%
\end{task}
\begin{task}[]\label{task-108}
The foci are at \((-1,3\pm 2)\) and vertices at \((-1, 3\pm 5)\).%
\end{task}
\end{exploration}
...
\begin{task}[]\label{task-109}
Show that \(d=2a\) by considering the difference of the distances from \(F_1\) and \(F_2\) to the vertex \((a,0)\).%
\end{task}
\begin{task}[]\label{task-110}
Let \(Q=(x,y)\) be a point on the hyperbola. By considering the difference of the distances between \(Q\) and the foci, show that an equation of the hyperbola is \(\frac{x^2}{a^2}-\frac{y^2}{c^2-a^2}=1,\) or if we let \(c^2-a^2=b^2\), then the equation is%
\begin{equation*}
\frac{x^2}{a^2}-\frac{y^2}{b^2}=1.
\end{equation*}
%
\end{task}
\begin{task}[]\label{task-111}
Suppose the foci are along the \(y\)-axis (at \((0,\pm c)\)) and the number \(d\) is now \(d=2b\), with vertexes \((0,\pm b)\). Let \(a^2=c^2-b^2\). Show that an equation of the hyperbola is%
\begin{equation*}
...
\begin{task}[]\label{task-112}
\(\ds 16x^2-25y^2=400\) divide by 400.%
%
\end{task}
\begin{task}[]\label{task-113}
\(\ds \frac{(x-1)^2}{5}-\frac{(y-2)^2}{9}=1\)%
\end{task}
\end{exploration}
...
\begin{task}[]\label{task-114}
The vertexes are at \((2\pm 3,1)\) and foci at \((2\pm 5, 1)\).%
\end{task}
\begin{task}[]\label{task-115}
The vertexes are at \((-1,3\pm 2)\) and foci at \((-1, 3\pm 5)\).%
\end{task}
\end{exploration}
...
Use the equation from \hyperref[arc_length_formula]{\ref{arc_length_formula}} to find the length of the curve \(\ds \vec r(t) = \left(t^3,\frac{3t^2}{2}\right)\) for \(t\in[1,3]\). The notation \(t\in[1,3]\) means \(1\leq t\leq 3\). Be prepared to show us your integration steps in class (you'll need a \(u\)-substitution).%
\end{exploration}
...
\begin{task}[]\label{task-135}
The parabola \(\vec p(t) = (t,t^2)\) for \(t\in[0,3]\).%
\end{task}
\begin{task}[]\label{task-136}
The ellipse \(\vec e(t) = (4\cos t,5\sin t)\) for \(t\in[0,2\pi]\).%
\end{task}
\begin{task}[]\label{task-137}
The hyperbola \(\vec h(t) = (\tan t,\sec t)\) for \(t\in[-\pi/ 4,\pi/4]\).%
\end{task}
\end{exploration}
To actually compute the integrals above and find the lengths, we would use a numerical technique to approximate the integral (something akin to adding up the areas of lots and lots of rectangles as you did in first semester calculus). \begin{aside}{Wrap Up}\label{aside-10}
...
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-11}
...
\typeout{************************************************}
\section[{Other Coordinate Systems}]{Other Coordinate Systems}\label{section-14}
...
\end{exploration}
In 3 dimensions, the most common coordinate systems are cylindrical and spherical. The equations for these coordinate systems are in the table below. \begin{aside}{}\label{aside-12}
\end{aside}
%
\begin{tabular}{ll}
...
\end{aside}
%
\begin{exploration}[]\label{exploration-101}
...
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-15}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-16}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
\leavevmode%
\begin{enumerate}
\item\hypertarget{li-48}{}Describe uses for, and construct equations or graphs of, space curves and parametric surfaces.%
\item\hypertarget{li-49}{}Find derivatives of space curves, and use this to find velocity, acceleration, and find equations of tangent lines.%
\item\hypertarget{li-50}{}Describe uses for, construct graphs of, and find the domain or range of functions of several variables.%
%
\begin{enumerate}
\item\hypertarget{li-51}{}For functions of the form \(z=f(x,y)\), this includes both 3D surface plots and 2D level curve plots.%
\item\hypertarget{li-52}{}For functions of the form \(w=f(x,y,z)\), construct plots of level surfaces.%
\end{enumerate}
\item\hypertarget{li-53}{}Describe uses for, and construct graphs of, vector fields and transformations.%
\item\hypertarget{li-54}{}Explain how to obtain a function for a vector field, or a parametrization for a curve or surface if you are given a description of the vector field, curve, or surface (instead of a function or parametrization),%
\end{enumerate}
\typeout{************************************************}
...
\typeout{************************************************}
\subsection[{Vector Fields and Transformations:}]{Vector Fields and Transformations:}\label{subsection-25}
...
\larsonfive{ }Consider the spherical coordinates transformation as defined in \stewarts{%
\begin{equation*}
\vec T(\rho,\phi,\theta)=(\rho\sin\phi\cos\theta,\rho\sin\phi\sin\theta,\rho\cos\phi),
\end{equation*}
%
\par
\begin{aside}{}\label{aside-17}
\end{aside}
%
\begin{equation*}
\vec T(\rho,\phi,\theta)=(\rho\sin\phi\cos\theta,\rho\sin\phi\sin\theta,\rho\cos\phi),
...
%
\par
Graphing this transformation requires \(3+3=6\) dimensions. In this problem we'll construct parts of this graph by graphing various surfaces. We did something similar for the polar coordinate transformation in \hyperref[polar_coordinate_transformation_graph]{problem~\ref{polar_coordinate_transformation_graph}}.%
\begin{task}[]\label{task-237}
Let \(\rho=2\) and graph the resulting surface.  What do you get if \(\rho = 3\)?%
\end{task}
\begin{task}[]\label{task-238}
Let \(\phi=\pi/4\) and graph the resulting surface.  What do you get if \(\phi=\pi/2\)?%
\end{task}
\begin{task}[]\label{task-239}
Let \(\theta=\pi/4\) and graph the resulting surface.  What do you get if \(\theta=\pi/2\)?%
\end{task}
\end{exploration}
\typeout{************************************************}
...
%
\par
Recall the spherical coordinate transformation as given in \stewarts{%
\begin{equation*}
\vec T(\rho,\phi,\theta) = (\rho\sin\phi\cos \theta, \rho\sin\phi\sin \theta,\rho \cos \phi).
\end{equation*}
%
...
\begin{task}[]\label{task-252}
Give a parametrization of the sphere of radius 2, using \(\phi\) and \(\theta\) as your input variables.%
\end{task}
\begin{task}[]\label{task-253}
What bounds should you place on \(\phi\) and \(\theta\) if you want to hit each point on the sphere exactly once? There are two possible answers here%
%
\end{task}
\begin{task}[]\label{task-254}
What bounds should you place on \(\phi\) and \(\theta\) if you only want the portion of the sphere above the plane \(z=1\)? There are also two possible answers here%
%
\end{task}
\end{exploration}
...
\begin{task}[]\label{task-369}
Compute both \(Df(x,y)\) and \(D\vec r(x)\) symbolically.  Don't use the function \(f(x,y)=x^2+3xy-y^3\) until the last step.%
\end{task}
\begin{task}[]\label{item_4}
Use the chain rule to compute \(D(f(\vec r(x)))\). What is \(dz/dx\) (i.e., \(df/dx\))?%
\end{task}
\begin{task}[]\label{task-371}
Since \(z\) is held constant, we know that \(dz/dx=0\). Use this fact, together with \hyperref[item_4]{part~\ref{item_4}} to explain why \(\ds \frac{dy}{dx} = -\frac{f_x}{f_y} = -\frac{\partial f/ \partial x}{\partial f/ \partial y}\).%
\end{task}
\begin{task}[]\label{task-372}
For the curve \(5=x^2+3xy-y^3\), use this formula to compute \(dy/dx\).%
\end{task}
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-19}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-20}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
\begin{task}[]\label{task-408}
Replace \(t\) with \(5t\) and compute \(\vec v\), \(\vec a\), \(\vec T\), \(\dfrac{d\vec T}{dt}\), \(\dfrac{d\vec T}{ds}\), and \(\left|\dfrac{d\vec T}{ds}\right|\).%
\end{task}
\begin{task}[]\label{task-409}
If we still want to evaluate at the time corresponding to the point \((0,3)\), what is \(t=\)?. \begin{hint}It should be smaller than \(\frac{\pi}{2}\)%
\end{task}
\begin{task}[]\label{task-410}
Replace \(t\) with \(\omega t\) and find \(\vec v\), \(\vec a\), \(\vec T\), \(\dfrac{d\vec T}{dt}\), \(\dfrac{d\vec T}{ds}\), and \(\left|\dfrac{d\vec T}{ds}\right|\).%
\end{task}
\begin{task}[]\label{task-411}
Explain why we should evaluate at \(t=\frac{\pi}{2\omega}\) for the point \((0,3)\).%
\end{task}
\begin{task}[]\label{task-412}
Evaluate each at \(t=\frac{\pi}{2\omega}\), the time corresponding to point \((0,3)\).%
\end{task}
\begin{task}[]\label{task-413}
...
\begin{exploration}[]\label{exploration-185}
\end{exploration}
We can use the above formula for curvature to get a quick way to compute the curvature of a function \(y=f(x)\). If you use the previous exercise, this formula falls out almost instantly. You'd see this formula in dynamics \begin{aside}{}\label{aside-22}
...
\typeout{************************************************}
\subsection[{Optional: Torsion}]{Optional: Torsion}\label{subsection-27}
...
%
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-24}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-25}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
This unit covers the following ideas. \begin{aside}{}\label{aside-26}
...
\end{aside}
 \leavevmode%
\begin{enumerate}
\item\hypertarget{li-117}{}Describe how to integrate a function along a curve. Use line integrals to find the area of a sheet of metal with height \(z=f(x,y)\) above a curve \(\vec r(t)=\left(x,y\right)\) and the average value of a function along a curve.%
\item\hypertarget{li-118}{}Find the following geometric properties of a curve: centroid, mass, center of mass, inertia, and radii of gyration.%
\item\hypertarget{li-119}{}Compute the work (flow, circulation) and flux of a vector field along and across piecewise smooth curves.%
\item\hypertarget{li-120}{}Determine if a field is a gradient field (hence conservative), and use the fundamental theorem of line integrals to simplify work calculations.%
\end{enumerate}
%
\par
\hyperref[line_integral_summary]{Table~\ref{line_integral_summary}} contains a summary of the key ideas for this chapter.%
\begin{table}
\centering
\begin{tabular}{ll}
&\tabularnewline\hrulethin
...
Suppose a wire \(C\) has the parameterization \(\vec r(t)\) for \(t\in[a,b]\). Suppose the wire's density at a point \((x,y,z)\) on the wire is given by the function \(\delta(x,y,z)\). \begin{aside}{}\label{aside-31}
...
\end{aside}
%
\begin{task}[]\label{task-559}
Consider a small portion of the wire at \(t=t_0\) of length \(ds\).  Explain why the mass of the small portion of the wire is \(dm=\delta(\vec r(t_0)) ds\).%
\end{task}
\begin{task}[]\label{task-560}
...
\typeout{************************************************}
\subsection[{Wrap Up}]{Wrap Up}\label{subsection-32}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
This unit covers the following ideas. \begin{aside}{}\label{aside-32}
...
\end{aside}
 \leavevmode%
\begin{enumerate}
\item\hypertarget{li-149}{}Explain the properties of the gradient, its relation to level curves and level surfaces, and how it can be used to find directional derivatives.%
\item\hypertarget{li-150}{}Find equations of tangent planes using the gradient and level surfaces. Use the derivative (tangent planes) to approximate functions, and use this in real world application problems.%
\item\hypertarget{li-151}{}Explain the second derivative test in terms of eigenvalues. Use the second derivative test to optimize functions of several variables.%
\item\hypertarget{li-152}{}Use Lagrange multipliers to optimize a function subject to constraints.%
\end{enumerate}
%
\par
...
\begin{task}[]\label{task-657}
What function \(f(x,y)\) do we wish to optimize? What is the constraint \(g(x,y)=c\)?%
\end{task}
\begin{task}[]\label{task-658}
...
\end{task}
\begin{task}[]\label{task-659}
At each ordered pair, find the temperature.%
\end{task}
\begin{task}[]\label{task-660}
...
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-38}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-39}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
This unit covers the following ideas. \begin{aside}{}\label{aside-40}
In preparation for the concept checks and exam, make sure you have a review packet containing examples that explain and illustrate the following concepts.%
\end{aside}
...
\end{aside}
 \leavevmode%
\begin{enumerate}
\item\hypertarget{li-174}{}Explain how to setup and compute a double integral. Show how to interchange the bounds of integration.%
\item\hypertarget{li-175}{}For planar regions, find area, mass, centroids, center of mass, moments of inertia, and radii of gyration.%
\item\hypertarget{li-176}{}Explain how to change coordinate systems in integration, in particular to polar coordinates.%
\item\hypertarget{li-177}{}Explain what the Jacobian is, and show how to use it.%
\item\hypertarget{li-178}{}Explain how to use Green's theorem to compute flow along and flux across a curve.%
\end{enumerate}
%
\par
...
\begin{exploration}[Radii of Gyration]\label{exploration-256}
\end{exploration}
...
\begin{task}[]\label{task-699}
Set up an integral formula which would give \(\bar y\) for the centroid of \(R\).  Then evaluate the integral.%
\end{task}
\begin{task}[]\label{task-700}
State \(\bar x\) from geometric reasoning. Sketch the region%
...
\begin{task}[]\label{task-703}
Compute \(\iint_R 3dA = 3\iint_RdA\).  [How can area help you?]%
\end{task}
\begin{task}[]\label{task-704}
Explain why \(\iint_R x dA = \bar x A\) for any region \(R\), and then compute \(\iint_R x dA\) for the circular disc. [You don't need to set up any integrals at all.]%
\end{task}
\begin{task}[]\label{task-705}
...
\end{task}
\begin{task}[]\label{task-708}
If the density depends only on \(x\), so \(\delta = \delta (x)\), set up an iterated integral formula to compute \(\bar y\) for the center of mass. Explain why%
\begin{equation*}
...
\end{task}
\end{exploration}
\begin{aside}{}\label{aside-45}
\end{aside}
%
\typeout{************************************************}
\typeout{Subsection 11.2.1 Optional: Application to Inertia}
\typeout{************************************************}
\subsection[{Optional: Application to Inertia}]{Optional: Application to Inertia}\label{subsection-34}
One of the main reasons we are studying mass, center of mass, centroids, etc., is so that we can understand energy. The transfer of energy (for example from kinetic to electrical and then back from electrical to kinetic) is one of the most important ideas in modern innovations.%
\par
Some of you may have already had a physics class, in which you learned that the kinetic energy of an object with mass \(m\) moving at speed \(v\) is%
\begin{equation*}
...
\end{equation*}
%
\begin{task}[]\label{task-719}
Write bounds for the region \(R\) by giving bounds for \(r\) and \(\theta\).%
\end{task}
\begin{task}[]\label{task-720}
Rewrite the double integral as an iterated integral using the bounds for \(r\) and \(\theta\). Don't forget the Jacobian (as \(dxdy=rdrd\theta\)).%
\end{task}
\begin{task}[]\label{task-721}
...
\end{exploration}
\begin{exploration}[Understanding Question]\label{exploration-274}
...
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-47}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-48}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
This unit covers the following ideas. \begin{aside}{}\label{aside-49}
In preparation for the concept checks and exam, make sure you have a review packet containing examples that explain and illustrate the following concepts.%
\end{aside}
...
\end{aside}
 \leavevmode%
\begin{enumerate}
\item\hypertarget{li-197}{}Explain how to setup surface integrals, and use them to compute surface area \begin{aside}{}\label{aside-51}
, average value, centroids, center of mass, moments of inertia, and radii of gyration.%
\end{aside}
%
\item\hypertarget{li-198}{}Use surface integrals to compute flux across a surface, in a given direction.%
\item\hypertarget{li-199}{}Explain how to use Stokes's theorem to compute circulation.%
\end{enumerate}
%
\par
...
\typeout{************************************************}
\subsection[{The Curl}]{The Curl}\label{subsection-37}
...
\end{task}
\end{exploration}
\begin{aside}{Wrap Up}\label{aside-52}
Once you have finished the problems in this \chaptername and feel comfortable with the ideas, create a short one page lesson plan that contains examples of the key ideas.  You will get a chance to teach from this lesson plan prior to taking the exam. Then log on to Brainhoney and download the quiz. Once you have taken the quiz, you can upload your work back to brainhoney and then download the key to see how you did. If you still need to work on mastering some of the ideas, please do so and then demonstrate your mastery though the quiz corrections.%
\end{aside}
 \begin{aside}{Wrap Up}\label{aside-53}
This concludes the \chaptername .  Look at the objectives at the beginning of the unit . Can you now do all the things you were promised?   \emph{Review Guide Creation} Your assignment: organize what you've learned into a small collection of examples that illustrates the key concepts. I'll call this your unit~review guide. I'll provide you with a template which includes the unit's key concepts from the objectives at the beginning. Once you finish your review guide, scan it into a PDF document (use any scanner on campus or photo software) and upload it to Gradescope.  As you create this review guide, consider the following: \leavevmode%
\begin{itemize}[label=\textbullet]
\item{}Before each Celebration of Knowledge  we will devote a class period to review. With well created lesson plans, you will have 4-8 pages(for 2-4 Chapters) to review for each, instead of 50-100 problems.%
\item{}Think ahead 2-5 years. If you make these lesson plans correctly, you'll be able to look back at your lesson plans for this semester. In about 20-25 pages, you can have the entire course summarized and easy for you to recall.%
...
\begin{task}[]\label{task-823}
An equation of an ellipse that passes through \((a,0)\) and \((0,b)\) is \(\ds\frac{x^2}{a^2}+\frac{y^2}{b^2}=1\).%
\end{task}
\begin{task}[]\label{task-824}
An equation of a line through these same two points is \(\ds\frac{x}{a}+\frac{y}{b}=1\).%
\end{task}
\begin{task}[]\label{task-825}
An equation of an ellipsoid through the three points \((a,0,0)\), \((0,b,0)\), and \((0,0,c)\) is \(\ds\frac{x^2}{a^2}+\frac{y^2}{b^2}+\frac{z^2}{c^2}=1\).%
\end{task}
\begin{task}[]\label{task-826}
...
\end{task}
\begin{task}[]\label{task-827}
Set up an iterated integral that would give the volume of \(D\).%
\end{task}
\begin{task}[]\label{task-828}
If the density is \(\delta(x,y,z) = 3x+2yz\), set up iterated integrals that would give the mass \(m\). \begin{aside}{}\label{aside-54}
...
Set up an integral in the coordinate system of your choice that would give the volume of the solid ball that is inside the sphere \(a^2=x^2+y^2+z^2\). Compute the integral to give a formula for the volume of a sphere of radius \(a\). \begin{aside}{}\label{aside-55}
