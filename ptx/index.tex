...
\leavevmode%
\begin{enumerate}[font=\bfseries,label=(\alph*),ref=\alph*]
\item\label{task-160} Let \(a=3\) and then graph the curve \(\vec T(3,\omega)=(3\cos\omega,9\sin\omega)\) for \(\omega\in[0,2\pi]\). You can start by making a \(\omega,x,y\) table.%
%
...
\item\label{task-162} \begin{enumerate}[font=\bfseries,label=(\roman*),ref=\theenumi.\roman*]
...
\item\label{task-828} If the density is \(\delta(x,y,z) = 3x+2yz\), set up iterated integrals that would give the mass \(m\). \begin{aside}{}\label{aside-54}
